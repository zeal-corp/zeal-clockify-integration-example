...
export async function createOrUpdateEmployeeCheck(
  ...
  if (!existingCheck) {
    ...
    return await exports.updateCheck(
...
export function buildShift(timeInterval: {
  end: string;
  duration: string;
}): any {
  const { end, duration } = timeInterval;
  const hours = ISO.parseHourlyShift(duration);
...
export async function createCheck(
  employeeID: string,
  reportingPeriod: any,
  hourlyShift: ZealParams.HourlyShift
) {
  const checkDate = setCheckDate(reportingPeriod);
  return await zealClient.createEmployeeCheck({
    companyID,
    employeeID,
...
export function setCheckDate(reportingPeriod: any): string {
  const ONE_WEEK = 7;
  const checkDate = ISO.adjustDateBy(reportingPeriod.end, ONE_WEEK);
